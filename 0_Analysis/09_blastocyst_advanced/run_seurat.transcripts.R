...
genes_s <- read.csv("results/cellcycle_s.transcript_name.tsv", header=F)$V1
genes_g2m <- read.csv("results/cellcycle_g2m.transcript_name.tsv", header=F)$V1
...
obj <- CellCycleScoring(obj, s.features = genes_s, g2m.features = genes_g2m, set.ident = TRUE)
obj <- RunPCA(obj, features = c(genes_s, genes_g2m))
DimPlot(obj)
...
new.cluster.ids <- c("ICM-2", "EPI", "TE-2", "PE", "ICM-1", "muralTE", "poralTE", "TE-1")
...
log-scaled synthesis/decay rates or total RNA levels (relative to the pluripotent state):